...
```
"console.log" will print the result to the console, allowing the result of a section of code to be viewed. "result" will give a function a value. Once the function has run, this value can be stored in a variable and used later.
```

**_What's the difference between defining and calling a function?_**

```
When a function is defined, the parameters are set and the logic for the arguments is provided. When the function is called, the arguments are provided that the function logic will use.
```
...
```
A string usually contains words or letters, but may also include numbers. Strings are contained in single quotes ('') or double quotes ("").
```

**_Number_**

```
A number includes a single unsigned integer, negative signed integer, unsigned float, or negative signed float.
```
...
```
An array contains several values contained in square brackets[]. Arrays may include any of the other data types, even other arrays.
```
